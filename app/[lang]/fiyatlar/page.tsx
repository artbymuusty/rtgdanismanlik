...
export async function generateMetadata({ params }: { params: LangParams }): Promise<Metadata> {
  ...
  return pageMetadata(lang, "pricing", { title: t.title, description: t.intro });
}

export default async function PricingPage({ params }: { params: LangParams }) {
  const lang = await getLang(params);
  ...
  return (
    <div className="py-16 sm:py-20">
          ...
          {t.tiers.map((tier, index) => (
            ...
            >
              ...
              <div className="flex flex-col justify-between gap-6">
                <div>
                  <p className="font-mono text-xs uppercase tracking-[0.1em] text-muted">{t.included}</p>
                ...
                <div className="flex flex-wrap items-center gap-4 border-t border-line pt-5">
                  ...
                  <Button href={applyHref} variant="secondary" className="!px-4 !py-2.5 text-sm">
                    {tier.cta}
                  </Button>
                </div>
              </div>
                  ...
                  {t.tiers.map((tier) => {
                    ...
                    return (
                      <td key={tier.id} className="px-4 py-3 text-accent">
                        {included ? (
                          <span aria-label={t.includedAria}>✓</span>
                        ) : (
      ...
      <Container className="mt-16 max-w-2xl rounded-[3px] border border-accent bg-paper-raised p-8 text-center">
        ...
        <div className="mt-6">
          <Button href={applyHref}>{t.cta.label}</Button>
        </div>
      </Container>
    </div>
  );
