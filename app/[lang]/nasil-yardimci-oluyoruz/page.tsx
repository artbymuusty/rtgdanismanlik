...
export async function generateMetadata({ params }: { params: LangParams }): Promise<Metadata> {
  ...
  return pageMetadata(lang, "services", { title: t.title, description: t.intro });
}
              ...
              >
                ...
                <div className={reverse ? "lg:order-1" : undefined}>
                  ...
                  <div className="mt-6 flex flex-col gap-5 border-t border-line pt-6">
                    <div>
                      <p className="font-mono text-xs uppercase tracking-[0.1em] text-muted">{t.labels.problem}</p>
                      <p className="mt-1 text-ink">{service.problem}</p>
                    </div>
                    <div>
                      <p className="font-mono text-xs uppercase tracking-[0.1em] text-muted">{t.labels.help}</p>
                      <p className="mt-1 text-ink">{service.help}</p>
                    </div>
                    <div>
                      <p className="font-mono text-xs uppercase tracking-[0.1em] text-muted">{t.labels.forWhom}</p>
                  ...
                  <div className="mt-6 grid grid-cols-1 gap-6 border-t border-line pt-6 sm:grid-cols-2">
                    <div>
                      <p className="font-mono text-xs uppercase tracking-[0.1em] text-muted">{t.labels.process}</p>
                    ...
                    <div>
                      <p className="font-mono text-xs uppercase tracking-[0.1em] text-muted">{t.labels.scope}</p>
                      <p className="mt-1 text-sm text-muted">{service.scope}</p>
                    </div>
                  </div>

                  <div className="mt-6">
                    <Button href={localizedPath(lang, "apply")} variant="secondary" className="!px-4 !py-2.5 text-sm">
                      {service.nextStep}
                    </Button>
                  </div>
                </div>
