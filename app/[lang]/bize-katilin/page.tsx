...
export async function generateMetadata({ params }: { params: LangParams }): Promise<Metadata> {
  ...
  const t = getDictionary(lang).bizeKatilin;
  return pageMetadata(lang, "joinUs", { title: t.title, description: t.intro });
}

export default async function JoinUsPage({ params }: { params: LangParams }) {
  const lang = await getLang(params);
  const dict = getDictionary(lang);
  const t = dict.bizeKatilin;
    ...
    <div className="py-16 sm:py-20">
      <Container className="grid grid-cols-1 items-center gap-10 lg:grid-cols-[1.1fr_1fr] lg:gap-16">
        ...
        <div className="relative">
          <EditorialPhoto alt={t.imageCaption} ratio="4 / 5" src="/images/people/mentor-consultation.jpg" priority />
      ...
      <Container className="mt-20 grid grid-cols-1 gap-10 lg:grid-cols-[0.8fr_1.2fr]">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.1em] text-gold">{t.applyEyebrow}</p>
        ...
        <MentorApplicationForm
          ...
          honeypotLabel={dict.common.honeypotLabel}
        />
      </Container>
    </div>
  );
